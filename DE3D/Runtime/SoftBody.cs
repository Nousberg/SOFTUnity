...
namespace DynamicEngine
{
    public class SoftBody : MonoBehaviour
    {
        ...
        [SerializeField, Range(0f, 0.1f)] public float areaCompliance = 0f;


        [SerializeField] public bool enablePlasticity = false;
        [SerializeField, Range(0f, 1f)] public float plasticityThreshold = 0.3f;
        [SerializeField, Range(0f, 1f)] public float plasticityAmount = 0.5f;

        [SerializeField] public bool enableCollision = true;
        [SerializeField] public float collisionRadius = 0.05f;
        [SerializeField] public LayerMask collisionLayers = ~0;

        [SerializeField] public bool enableDeformation = true;
        [SerializeField] public bool enableSelfCollision = false;  // V2V self-collision within same body
        ...
        private void OnValidate()
        {
            ...
            {
                solver.internalPressure = internalPressure;
                solver.areaCompliance = areaCompliance;
                ApplyCollisionSettingsToSolver();
            }
        }
        ...
        private void FixedUpdate()
        {
            ...
            // Prevent multiple updates in the same physics step
            if (s_lastUpdateFrame == Time.frameCount) return;
        ...
        {
            int numSubSteps = SceneSettings.Instance != null ? SceneSettings.Instance.BaseSubSteps : 1;
            float timeScale = SceneSettings.Instance != null ? SceneSettings.Instance.SimulationTimeScale : 1.0f;
            float subDt = (Time.fixedDeltaTime * timeScale) / numSubSteps;

            for (int step = 0; step < numSubSteps; step++)
            {
                // 1. All bodies Predict (Gravity applied to everyone first)
                foreach (var sb in AllSoftBodies)
                    if (sb.solver != null) sb.solver.SubStep_PrepareAndIntegrate(subDt);

                // 2. All bodies Resolve (Cross-body links now see updated predictions)
                foreach (var sb in AllSoftBodies)
                    if (sb.solver != null) sb.solver.SubStep_ResolveConstraints(subDt);

                // 3. All bodies Finalize
                foreach (var sb in AllSoftBodies)
                    if (sb.solver != null) sb.solver.SubStep_Finalize();
            }

            // 4. Mesh Deformation (Can be done once after all sub-steps)
            foreach (var sb in AllSoftBodies)
        ...
        private bool InitializeCore()
        {
            ...
            solver = new Solver(1f, mesh, mesh.vertices, this);
        ...
        private void ApplyCollisionSettingsToSolver()
        {
            if (solver == null) return;
            solver.ApplyCollisionSettings(collisionRadius, collisionLayers);
        }
        ...
        /// <summary>
        /// Resets all nodes to their initial positions, restoring the original shape.
        /// Can only be called during runtime (Play mode).
        /// </summary>
        public void ResetDeformation()
        {
            if (!Application.isPlaying)
            {
                Debug.LogWarning("[SoftBody] ResetDeformation can only be called during Play mode.");
                return;
            }

            if (solver == null)
            {
                Debug.LogWarning("[SoftBody] Cannot reset deformation: Solver not initialized.");
                return;
            }

            solver.RestoreNodesAndBeams(transform);
            Debug.Log("[SoftBody] Deformation reset to initial state.");
        }
    }
}
