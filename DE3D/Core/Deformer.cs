...
namespace DynamicEngine
{
    public class MeshDeformer
    {
        ...
        // Fixed: Persistent Node Data Buffers to prevent leaks in high-frequency updates
        ...
        // Track main thread for safe disposal
                       ...
                       IReadOnlyList<Vector3> initialPositions, IReadOnlyList<Quaternion> initialRotations)
        {
            ...
            // Fixed: SetData to persistent buffers instead of 'new ComputeBuffer'
        ...
        ~MeshDeformer()
        {
            // Note: GPU resources cannot be safely released in finalizer, Unity handles them on shutdown.
            // NativeArrays are persistent so they are cleaned up if still present.
