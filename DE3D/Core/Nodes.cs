...
namespace DynamicEngine
{
    public class NodeManager
    {
        ...
        {
            baseRadius = Mathf.Clamp(radius, 0.001f, 1f);

            for (int i = 0; i < nodeColliders.Count; i++)
            {
                if (nodeColliders[i] != null)
                {
                    nodeColliders[i].radius = baseRadius;
                }
            }

            if (nativeArraysAllocated && nativeRadii.IsCreated)
            {
                for (int i = 0; i < nativeRadii.Length; i++)
                {
